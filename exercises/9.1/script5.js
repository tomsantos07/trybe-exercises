...
const sendMarsTemperature = (onSuccess, onError)  => {
  let showTemperature = getMarsTemperature();
  let rateSuccess = Math.random();
  if (rateSuccess < 0.4) setTimeout(() =>{ onError(rateSuccess) }, messageDelay());
  else setTimeout(() =>{ onSuccess(showTemperature) }, messageDelay());
};
